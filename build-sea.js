...
async function buildSEA() {
  ...
  try {
    ...
    for (const platform of platforms) {
      ...
      const outputName = platform.name === 'win' ? `weebsync-${platform.name}.exe` : `weebsync-${platform.name}`;
      ...
      if (existsSync(platform.nodeExe)) {
        ...
        // Inject the blob
        await runCommand('npx', ['postject', outputPath, 'NODE_SEA_BLOB', 'sea-prep.blob', 
          '--sentinel-fuse', 'NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2']);
        
        console.log(`Built ${outputPath}`);
      } else {
        console.warn(`Node.js executable not found for ${platform.name}: ${platform.nodeExe}`);
        console.warn(`Download Node.js binaries manually and place them in the project root.`);
      }
    }
